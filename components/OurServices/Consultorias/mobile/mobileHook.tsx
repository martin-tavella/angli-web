...
const ConsultoriasMobileHook = ({
  showMobileHook,
  handleOnClick,
}: MobileHookProps) => {
  return (
    <article
      className={` mx-auto rounded-b-3xl md:max-w-[80%]
      transition-all duration-800 ease-in-out overflow-hidden
      ] border-[#d85254]
      ${
        showMobileHook
          ? "max-h-[1300px] sm:max-h-[1350px] md:max-h-[1600px] lg:max-h-[1150px] border-[5px]"
    ...
    >
      ...
      >
        <div className="max-w-[90%] mx-auto pt-10">
          ...
          <p className="uppercase text-[#ee6226] text-center text-[14px] sm:text-[16px] md:text-xl font-light max-w-[70%] mx-auto py-6 sm:pb-10">
        ...
        <div className="flex relative pt-3 pb-10">
           <button onClick={handleOnClick} className="z-50 cursor-pointer mx-auto absolute bottom-1 -right-8 sm:-right-4 hover:scale-105 transition-transform duration-300">
            <Image
              src={reducir.src}
              alt="Contacto"
              width={211}
...
export default ConsultoriasMobileHook;
